        ...
        <div className={`flex  ${flexResponsive} max-h-full flex-auto pb-2 mb-2 border-b border-neutral-400 gap-x-4`}>
          {props.componentData.map((item: any) => (
            <div className='flex-auto mb-2 ' key={item.id}>
              ...
              <input
                className={`focus:outline-none focus:shadow-outline shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight
                ${errors[item.field] ? 'border-red-500' : ''} ${item.type==='disable' ? 'cursor-not-allowed bg-gray-200' : ''}`}
                type={item.type}
                step={item.type === "number" ? "0.01" : undefined}
                value={props.items[item.field]}
                {...register(item.field)} />
              {errors[item.field] && (
